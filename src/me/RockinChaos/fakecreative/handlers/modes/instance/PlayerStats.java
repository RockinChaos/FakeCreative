...
public class PlayerStats {
    ...
    */
	public PlayerStats(final Player player) {
		if (player != null && this.isLocalePreferences(player)) {
			final DataObject allowFlight = SQL.getData().getData(new DataObject(Table.ALLOW_FLIGHT, PlayerHandler.getPlayerID(player), true));
			if (allowFlight != null) {
				this.allowFlight = allowFlight.getBoolean();
			}
		
			final DataObject flySpeed = SQL.getData().getData(new DataObject(Table.SPEED_FLIGHT, PlayerHandler.getPlayerID(player), 0.0));
			if (flySpeed != null) {
				this.flySpeed = flySpeed.getDouble();
			}
			
			final DataObject breakSpeed = SQL.getData().getData(new DataObject(Table.SPEED_BREAK, PlayerHandler.getPlayerID(player), 0.0));
			if (breakSpeed != null) {
				this.breakSpeed = breakSpeed.getDouble();
			}

			final DataObject foodLevel = SQL.getData().getData(new DataObject(Table.SET_FOOD, PlayerHandler.getPlayerID(player), 0));
			if (foodLevel != null) {
				this.foodLevel = foodLevel.getInt();
			}

			final DataObject health = SQL.getData().getData(new DataObject(Table.SET_HEALTH, PlayerHandler.getPlayerID(player), 0));
			if (health != null) {
				this.health = health.getInt();
			}

			final DataObject heartScale = SQL.getData().getData(new DataObject(Table.SET_SCALE, PlayerHandler.getPlayerID(player), 0.0));
			if (heartScale != null) {
				this.heartScale = heartScale.getDouble();
			}

			final DataObject allowHunger = SQL.getData().getData(new DataObject(Table.ALLOW_HUNGER, PlayerHandler.getPlayerID(player), true));
			if (allowHunger != null) {
				this.allowHunger = allowHunger.getBoolean();
			}

			final DataObject allowBurn = SQL.getData().getData(new DataObject(Table.ALLOW_BURN, PlayerHandler.getPlayerID(player), true));
			if (allowBurn != null) {
				this.allowBurn = allowBurn.getBoolean();
			}

			final DataObject unbreakableItems = SQL.getData().getData(new DataObject(Table.UNBREAKABLE_ITEMS, PlayerHandler.getPlayerID(player), true));
			if (unbreakableItems != null) {
				this.unbreakableItems = unbreakableItems.getBoolean();
			}

			final DataObject blockDrops = SQL.getData().getData(new DataObject(Table.DROPS_BLOCK, PlayerHandler.getPlayerID(player), true));
			if (blockDrops != null) {
				this.blockDrops = blockDrops.getBoolean();
			}

			final DataObject swordBlock = SQL.getData().getData(new DataObject(Table.SWORD_BLOCK, PlayerHandler.getPlayerID(player), true));
			if (swordBlock != null) {
				this.swordBlock = swordBlock.getBoolean();
			}

			final DataObject autoRestore = SQL.getData().getData(new DataObject(Table.AUTO_RESTORE, PlayerHandler.getPlayerID(player), true));
			if (autoRestore != null) {
				this.autoRestore = autoRestore.getBoolean();
			}

			final DataObject god = SQL.getData().getData(new DataObject(Table.SET_GOD, PlayerHandler.getPlayerID(player), true));
			if (god != null) {
				this.god = god.getBoolean();
			}
			
			final DataObject godDelay = SQL.getData().getData(new DataObject(Table.DELAY_GOD, PlayerHandler.getPlayerID(player), 0));
			if (godDelay != null) {
		...
		for (int i = 1; i <= 9; i++) {
			final DataObject dataObject = SQL.getData().getData(new DataObject(Table.HOTBAR, PlayerHandler.getPlayerID(player), Integer.toString(i), "64"));
			if (dataObject != null) {
    ...
    */
    public void setFlight(final Player player, final boolean allowFlight) {
    	synchronized ("FK_SQL") {
	    	...
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.ALLOW_FLIGHT, PlayerHandler.getPlayerID(player), allowFlight));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.ALLOW_FLIGHT, PlayerHandler.getPlayerID(player), allowFlight));
    	...
    	synchronized ("FK_SQL") {
	    	...
	    	SchedulerUtils.run(() -> { 
				if (ServerUtils.hasSpecificUpdate("1_8")) {
					player.getAttribute(org.bukkit.attribute.Attribute.GENERIC_MAX_HEALTH).setBaseValue(((heartScale) * 2)); 
				} else {
    ...
    */
    public void setAllowHunger(final Player player, final boolean allowHunger) {
    	synchronized ("FK_SQL") {
	    	...
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.ALLOW_HUNGER, PlayerHandler.getPlayerID(player), allowHunger));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.ALLOW_HUNGER, PlayerHandler.getPlayerID(player), allowHunger));
    	}
    }
    ...
    */
    public void setAllowBurn(final Player player, final boolean allowBurn) {
    	synchronized ("FK_SQL") {
	    	...
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.ALLOW_BURN, PlayerHandler.getPlayerID(player), allowBurn));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.ALLOW_BURN, PlayerHandler.getPlayerID(player), allowBurn));
			if (!allowBurn) {
				player.setFireTicks(0);
			} else {
				player.setFireTicks(PlayerHandler.getCreative(player).getFireTicks());
			} 
    	}
    }
    ...
    */
    public void setUnbreakableItems(final Player player, final boolean unbreakableItems) {
    	synchronized ("FK_SQL") {
	    	...
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.UNBREAKABLE_ITEMS, PlayerHandler.getPlayerID(player), unbreakableItems));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.UNBREAKABLE_ITEMS, PlayerHandler.getPlayerID(player), unbreakableItems));
    	}
    }
    ...
    */
    public void setBlockDrops(final Player player, final boolean blockDrops) {
    	synchronized ("FK_SQL") {
	    	...
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.DROPS_BLOCK, PlayerHandler.getPlayerID(player), blockDrops));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.DROPS_BLOCK, PlayerHandler.getPlayerID(player), blockDrops));
    	}
    }
    ...
    */
    public void setSwordBlock(final Player player, final boolean swordBlock) {
    	synchronized ("FK_SQL") {
	    	...
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.SWORD_BLOCK, PlayerHandler.getPlayerID(player), swordBlock));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.SWORD_BLOCK, PlayerHandler.getPlayerID(player), swordBlock));
    	}
    }
    ...
    */
    public void setRestore(final Player player, final boolean autoRestore) {
    	synchronized ("FK_SQL") {
	    	...
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.AUTO_RESTORE, PlayerHandler.getPlayerID(player), autoRestore));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.AUTO_RESTORE, PlayerHandler.getPlayerID(player), autoRestore));
    	}
    }
    ...
    */
    public void setGod(final Player player, final boolean god) {
    	synchronized ("FK_SQL") {
	    	this.god = god;
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.SET_GOD, PlayerHandler.getPlayerID(player), god));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.SET_GOD, PlayerHandler.getPlayerID(player), god));
			if (ServerUtils.hasSpecificUpdate("1_8")) {
    ...
    */
    public void setStore(final Player player, final boolean storeInventory) {
    	synchronized ("FK_SQL") {
	    	...
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.STORE_INVENTORY, PlayerHandler.getPlayerID(player), storeInventory));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.STORE_INVENTORY, PlayerHandler.getPlayerID(player), storeInventory));
	    	if (!storeInventory) {
	    		PlayerHandler.restoreInventory(player); {
	    			PlayerHandler.getCreative(player).setInventory64(new String());
	    		}
	    	} else {
    ...
    */
    public void saveHotbar(final Player player, final String inventoryData, final int hotbar) {
    	synchronized ("FK_SQL") {
	    	...
	    	final DataObject dataObject = SQL.getData().getData(new DataObject(Table.HOTBAR, PlayerHandler.getPlayerID(player), Integer.toString(hotbar), ""));
	    	if (dataObject != null) { SQL.getData().removeData(dataObject); }
	    	SQL.getData().saveData(new DataObject(Table.HOTBAR, PlayerHandler.getPlayerID(player), Integer.toString(hotbar), inventoryData));
    	}
    }
